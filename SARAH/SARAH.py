...
# Dependency PATH
gatk = '/mnt/lustre/software/gatk/gatk'
bamtools = '/home/song7602/miniconda3/bin/bamtools'
cooler = '/home/song7602/miniconda3/bin/cooler'
ToolDir = sys.path[0]
...
def parse_arguments():
	...
	optional.add_argument('-r', '--RES',
                              type=str,
			      default='5kb',
                              help='Resolutions for hi-c contact map (5kb, 10kb, 25kb)')
	optional.add_argument('-t', '--THREADS',
			      default=10,
			      type=int,
                              help='Number of threads for multi-processing (default=20)')
	optional.add_argument('-d', '--DATA',
			      default='WGS',
                              help='Data type of input bam file (WGS, HiC)')
	optional.add_argument('-s', '--SCORE',
                              default=100,
			      type=int,
...
def longest_block(hap):
	...
	for line in open_hap:
		...
		if line[0] == 'B':
			...
			phased = int(col[6])
			block_numb += 1
			block_key = (f'{hap}_{block_numb}', span, phased)
			block_dict[block_key] = [line]
		elif line[0] == '*':
			continue
		else:
			block_dict[block_key].append(line)
		del line
	### 2-2. get longest block ###
	# longest haplotype blocks for phasing
	longest_block = sorted(block_dict.keys(), key=lambda x: x[1], reverse = True)[0]
	block_len = longest_block[1]
	major_hap = f'{hap}_major_haplotype'
	out_hap = open(major_hap, 'w')
	for line in block_dict[longest_block]:
		out_hap.write(line + '\n')
	out_hap.close()
	del block_dict
...
def SplitBam(bam, hap, process_dir, chrom, pos, ref, alt, order, score):
	hap_name = hap.split('/')[-1]
	a_bam = f'{process_dir}/A.{hap_name}.{chrom}.{pos}.bam'
	b_bam = f'{process_dir}/B.{hap_name}.{chrom}.{pos}.bam'
	c_bam = f'{process_dir}/C.{hap_name}.{chrom}.{pos}.bam'
	_pos = pos - 1
...
# merging bam files 
def merge_bam(allele, hap_name, input_phased_split_bam, numb, process_dir,bamtools): # for limiting the number of files per bamtools process
	...
	bam_0 = f'{process_dir}/{allele}.{hap_name}.bam_0.bam.numb:{numb}.bam'
...
def Sarah(hap, bam, chrom, name, score, multi_thread, data_type, bamtools):
	print_log('Building SNP-Overlap Bam Files')

	chr_dir = '/'.join(bam.split('/')[:-1])
	chr_bam = bam.split('/')[-1] # input2: chr_bam
	open_hap = open(hap, 'r')
	hap_name = hap.split('/')[-1]
	if data_type == 'HiC':
	...
	samfile = pysam.AlignmentFile(f'{chr_dir}/{chr_bam}')
	phased_bam_list = []
	hap_line = []
	allele_numb = 0
	for line in open_hap:
		...
		if line[0] not in ["*", "B"]:
			if '-' not in [line.split()[1], line.split()[2]]:
				...
				if hap_score >= score:
					...
					alt = col[6]
					order = col[1:3]
					phased_bam = f'{hap_name}.{chrom}.{pos}.sorted.bam'
					phased_bam_list.append(phased_bam)
					print_log(f'{chrom}\t{pos}')
					SplitBam(bam, hap, process_dir, chrom, pos, ref, alt, order, score)
	print_log('done')
	#### Merging Bam Files ####
	...
	for numb in phased_bam_for_merge:
		merge_bam('A', hap_name, phased_bam_for_merge[numb], numb, process_dir, bamtools)
		check_if_finished(f'{process_dir}/A.{hap_name}.bam_0.bam.numb:{numb}.bam', 'slow down...')
		merge_bam('B', hap_name, phased_bam_for_merge[numb], numb, process_dir, bamtools)
		check_if_finished(f'{process_dir}/B.{hap_name}.bam_0.bam.numb:{numb}.bam', 'slow down...')
		merge_bam('C', hap_name, phased_bam_for_merge[numb], numb, process_dir, bamtools)
		check_if_finished(f'{process_dir}/C.{hap_name}.bam_0.bam.numb:{numb}.bam', 'slow down...')

	A_merged_bam0 = f'{process_dir}/A.{hap_name}.bam_0.bam'
	B_merged_bam0 = f'{process_dir}/B.{hap_name}.bam_0.bam'
	C_merged_bam0 = f'{process_dir}/C.{hap_name}.bam_0.bam'

	if data_type == 'HiC':
		rmdup_bam = f'{hap_name}.hic.bam'
	else:
		rmdup_bam = f'{hap_name}.wgs.bam'
...
def GetMate(hap, chrom_dir, bam, phased_bam, read_chrom, mate_chrom, process_dir):
		...
		if -1 not in [read.tid == read.rnext] and read.is_read1:
			if read.flag & 1 and read.flag & 4 == read.flag & 8 == read.flag & 256 == read.flag & 1024 == 0:
				...
				if next_ref == mate_chrom:
					mate_bam = f'{mate_chrom}.{bam_name}'
					open_mate_bam = pysam.AlignmentFile(f'{chrom_dir}/{mate_bam}', 'rb')
					try:
						mate_read = open_mate_bam.mate(read)
...
mate_list = ['chr1','chr2','chr3','chr4','chr5','chr6','chr7','chr8','chr9',
	...
	'chr18','chr19','chr20','chr21','chr22', 'chrX', 'chrY', 'HPV35']

def get_chrom_mate(gatk, bam, hap, read_chrom, name):
	process_dir = f'{read_chrom}.{name}.phasing.hic.dir'
	chrom_dir = bam.split('/')[0]
	hap_name = hap.split('/')[-1]
	print_log('SARAH Get_Mate_Reads_By_Chromosomes Initializes')
	print_log('Getting Mate Reads Starting...')

	hap_bam = f'{hap_name}.hic.bam'
	A_bam = 'A.' + hap_bam
	B_bam = 'B.' + hap_bam
	C_bam = 'C.' + hap_bam
	out_list_a = []
	...
	for mate_chrom in mate_list:
		threads.append(Thread(target=GetMate, args=(hap, chrom_dir, bam, A_bam, read_chrom, mate_chrom, process_dir)))
		threads.append(Thread(target=GetMate, args=(hap, chrom_dir, bam, B_bam, read_chrom, mate_chrom, process_dir)))
		threads.append(Thread(target=GetMate, args=(hap, chrom_dir, bam, C_bam, read_chrom, mate_chrom, process_dir)))
	...
	print_log(f"Merging Block {hap_name}'s Bam Files Finished")
	print_log(f"Getting Mate Reads Of Block {hap_name} is Done")
...
def bam2cool(bam, res):
	...
	if res == '5kb':
		zoomify.append('5000,10000,25000,100000,200000,500000,1000000,2000000,5000000')
	elif res == '10kb':
		zoomify.append('10000,20000,100000,200000,500000,1000000,2000000,5000000')
	elif res == '25kb':
...
def final_bam(allele, hap):
	hap_name = hap.split('/')[-1]
	return f'{allele}.{hap_name}.hic.get.mate.merged.sort.bam'

if __name__ == '__main__':
	args = parse_arguments()
	longest_block(args.HAPLOTYPE)
	Sarah(args.HAPLOTYPE, args.BAM, args.CHROM, args.NAME, args.SCORE, args.THREADS, args.DATA, bamtools)
	if args.DATA == 'HiC':
		get_chrom_mate(gatk, args.BAM, args.HAPLOTYPE, args.CHROM, args.NAME)
		threads = []
		threads.append(Thread(target=bam2cool, args=(final_bam('A', args.HAPLOTYPE), args.RES)))
		threads.append(Thread(target=bam2cool, args=(final_bam('B', args.HAPLOTYPE), args.RES)))
		threads.append(Thread(target=bam2cool, args=(final_bam('C', args.HAPLOTYPE), args.RES)))
		for thread in threads:
			thread.start()
		for thread in threads:
			thread.join()
